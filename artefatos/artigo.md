...
O objetivo central deste trabalho foi desenvolver um pipeline reprodutível para a definição de limites de crédito pré-aprovados em larga escala, capaz de maximizar o retorno líquido esperado da carteira enquanto respeita restrições prudenciais e operacionais explícitas. A análise dos resultados obtidos permite avaliar em que medida esse objetivo foi alcançado em cada uma das etapas do pipeline.
 
A etapa de segmentação via CART produziu aproximadamente $K = 800$ clusters a partir da base elegível, particionando clientes com perfis heterogêneos de risco, propensão e capacidade de pagamento em grupos internamente homogêneos. A escolha de $K = 800$ foi justificada empiricamente por varredura de ganho marginal: acima desse valor, o ganho adicional de homogeneidade dentro dos clusters não compensa o aumento de complexidade do problema de Programação Linear. Esse resultado é consistente com a expectativa metodológica de que árvores de decisão do tipo CART, ao particionar o espaço de variáveis por cortes binários recursivos, tendem a produzir grupos que diferem substantivamente em pelo menos uma variável relevante ao modelo — no caso, `pd_calibrada`, `score_propensao_contrato` e `cp_proxy`.
 
A formulação do Problema de Programação Linear com os parâmetros agregados por cluster produziu um problema com $K$ variáveis de decisão e um conjunto de restrições lineares que reflete fielmente as diretrizes operacionais do parceiro: teto de inadimplência financeira ponderada por exposição (R1), alavancagem por capacidade de pagamento com multiplicador diferenciado por `score_credito_cross` (R2) e teto operacional de limite por oferta (R3). A implementação própria do Simplex com regra de Bland convergiu para a solução ótima nas instâncias testadas, e sua validação cruzada com o solver CBC via PuLP confirmou consistência nos valores da função objetivo e no vetor de decisão $L_k^*$, com discrepâncias residuais dentro das tolerâncias numéricas esperadas.
 
Do ponto de vista da função objetivo, o coeficiente marginal $c_k = \pi_k \cdot (T\bar{u}t - PD_k \cdot \mathrm{LGD})$ captura adequadamente o trade-off risco-retorno por cluster: clusters com alta propensão à contratação ($\pi_k$ elevado) e baixa probabilidade de inadimplência ($PD_k$ reduzida) tendem a receber limites maiores na solução ótima, ao passo que clusters com perfil de risco elevado recebem limites mais contidos ou nulos — o que é economicamente coerente. Essa dinâmica é diretamente observável na distribuição dos $L_k^*$ obtidos: a concentração de ofertas ativas em clusters com $c_k > 0$ e a exclusão automática de clusters com $c_k \leq 0$ refletem o comportamento esperado de um otimizador linear sem termo de regularização.
 
A restrição R1 — teto de inadimplência financeira ponderada pela exposição total — mostrou-se a mais frequentemente ativa na solução ótima, o que indica que, sem essa limitação, o otimizador tenderia a concentrar exposição em clusters de maior retorno marginal bruto, mas com $PD$ mais elevada. Esse comportamento é esperado e desejável do ponto de vista do modelo: a restrição cumpre seu papel de âncora prudencial, impedindo que a maximização de retorno degrade a qualidade da carteira abaixo do patamar de risco aceito pela política vigente.
 
O pós-processamento dos limites contínuos $L_k^*$ para múltiplos de R\$ 50 e a exclusão de ofertas abaixo de R\$ 200 introduziram pequenas variações em relação ao ótimo contínuo, porém necessárias para aderência operacional. A verificação da diretriz de concentração por cluster (R4) na etapa de pós-otimização sinaliza eventuais clusters que, mesmo após discretização, concentram parcela excessiva da exposição total, permitindo ao operador ajustar o parâmetro $\alpha$ e reexecutar o pipeline de forma controlada.
 
### 5.2 Comparação com trabalhos relacionados
 
A comparação dos resultados obtidos com os trabalhos analisados na revisão de literatura evidencia tanto as convergências metodológicas quanto os avanços propostos por este pipeline.
 
Em relação a Al-Musbahu et al. (2025), a semelhança estrutural está no uso da Programação Linear para otimização de carteiras de crédito com dados reais de instituição financeira. Ambos os trabalhos formulam restrições explícitas que incorporam regras de negócio — percentuais mínimos por categoria no caso de Al-Musbahu, e teto de inadimplência ponderada e alavancagem por capacidade no presente trabalho — e demonstram que a solução ótima aloca zero para as categorias ou clusters economicamente não atrativos. A diferença central está na granularidade: enquanto Al-Musbahu et al. (2025) opera sobre um número reduzido de categorias de empréstimo predefinidas institucionalmente, este trabalho opera sobre aproximadamente 800 clusters derivados dos próprios dados, o que permite calibrar limites de forma proporcional ao perfil individual de cada grupo de clientes. Em escala de portfólio com milhões de clientes, essa diferença de granularidade tem impacto direto sobre a precisão da estimativa de retorno e sobre a capacidade do modelo de diferenciar a oferta segundo características observadas.
 
A comparação com Kwapong (2013) é a mais reveladora do ponto de vista estrutural. A função objetivo de Kwapong — $Z = \sum_j I_j(1 - P_j)x_j$ — é a expressão mais próxima do coeficiente $c_k = \pi_k \cdot (T\bar{u}t - PD_k \cdot \mathrm{LGD})$ adotado neste trabalho: em ambos os casos, o retorno por unidade alocada é calculado como a diferença entre receita esperada e perda esperada por inadimplência, e a variável de decisão é o volume de crédito a ser concedido. A análise de sensibilidade conduzida por Kwapong em sete cenários, identificando restrições ativas por meio de preços duais, tem correspondência direta na verificação das restrições ativas do PL na etapa de pós-otimização deste trabalho. O avanço proposto aqui está na substituição de categorias predefinidas por clusters endógenos derivados por CART, o que permite capturar a heterogeneidade real da base de clientes e atribuir limites contínuos individualizados em vez de volumes agregados por modalidade.
 
A comparação com Li, Tao e Li (2022) evidencia a complementaridade entre os dois trabalhos. Li et al. demonstram que dados financeiros conseguem separar perfis de risco com objetividade — resultado que sustenta a premissa da etapa de segmentação deste pipeline. A diferença essencial está no que se faz após a segmentação: Li et al. encerram o trabalho na identificação dos grupos de risco, sem traduzir essa informação em nenhuma decisão operacional. O presente trabalho utiliza a segmentação como etapa intermediária que viabiliza a formulação do PL, avançando do diagnóstico para a prescrição. Esse é, precisamente, o salto que o pipeline proposto adiciona em relação aos três trabalhos analisados: integrar segmentação baseada em dados e otimização prescritiva em um pipeline único, aplicado a crédito ao consumidor em escala real.
 
A referência a Scarpel e Milioni (2002), citada na lacuna identificada, reforça a legitimidade do paradigma de duas etapas — estimar e depois otimizar — no contexto brasileiro, ainda que seu problema seja de natureza binária sobre crédito corporativo. O presente trabalho pode ser interpretado como uma extensão desse paradigma para o domínio de limite contínuo em pessoa física, com segmentação endógena via CART e restrições mais ricas derivadas do contexto operacional do parceiro.
 
### 5.3 Discussão dos resultados e possíveis aplicações
 
Os resultados obtidos sugerem que a abordagem proposta é tecnicamente viável e operacionalmente integrável, mas apontam também para dimensões que merecem atenção na aplicação prática.
 
Do ponto de vista da política de concessão, o pipeline permite comparar diretamente a recomendação do modelo com o `limite_ofertado` da política vigente, disponível como baseline na base de dados. Essa comparação viabiliza o backtesting: é possível avaliar, para os clientes que contrataram o produto (`flag_contrato = 1`) e o ativaram (`flag_ativacao = 1`), se os limites recomendados pelo modelo teriam gerado retorno superior ou inferior ao observado historicamente, e se a inadimplência observada (`over30mob3`) teria sido contida dentro das restrições impostas. Embora o viés de seleção inerente à amostra de contratantes — que representa apenas 0,04% da base elegível — limite a generalização direta desse backtesting, ele fornece uma âncora empírica para calibrar os parâmetros do modelo em ciclos subsequentes.
 
Uma aplicação imediata do pipeline é o uso como ferramenta de parametrização periódica da política de oferta. Com a estrutura de endpoint HTTP disponibilizada via FastAPI, o pipeline pode ser integrado a rotinas internas de atualização mensal de limites: uma nova safra de dados alimenta o pipeline, os parâmetros operacionais são ajustados via arquivo JSON e os limites recomendados por cluster são disponibilizados para o sistema de oferta. A rastreabilidade das decisões — garantida pelo registro do `cluster_id` por cliente e dos parâmetros utilizados em cada execução — atende ao requisito de auditabilidade mapeado pelo parceiro.
 
Do ponto de vista das limitações reconhecidas, duas merecem destaque. A primeira é a estabilidade dos clusters ao longo do tempo: como o CART é treinado sobre uma safra específica, a composição dos clusters pode mudar entre safras caso o comportamento da base evolua, exigindo revalidação periódica do modelo de segmentação. A segunda é a dependência da qualidade dos parâmetros estimados: os coeficientes $c_k$ são funções de $PD_k$, $\pi_k$ e $CP_k$, todos estimados com incerteza. Em particular, a calibração da PD por decis, embora mitigue o viés de seleção inerente à aprovação histórica, introduz extrapolação nos decis de maior risco — onde a observação de `over30mob3` é escassa —, o que pode subestimar o risco real nesses grupos. Uma extensão natural do trabalho seria incorporar intervalos de confiança nos parâmetros estimados, formulando um problema de otimização robusta em lugar do PL determinístico adotado.
 
Mais amplamente, o pipeline proposto pode ser adaptado a outros produtos de crédito pré-aprovado com estrutura similar — crédito pessoal, cheque especial, crédito consignado — desde que os parâmetros operacionais e as restrições prudenciais sejam recalibrados para o produto em questão. A modularidade do pipeline, com separação explícita entre pré-processamento, segmentação e otimização, facilita essa adaptação sem necessidade de reformulação completa do modelo.
