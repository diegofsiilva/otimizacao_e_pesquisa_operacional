...
**Aderência às restrições.** O resultado mais relevante sobre as restrições é que a solução ótima em M1 foi inteiramente governada pela restrição de capacidade de pagamento (R2), ativa em 677 dos 800 clusters. O teto de risco financeiro agregado (R1) permaneceu com folga expressiva: a PD ponderada por exposição da carteira otimizada ficou em 0,067, bem abaixo do teto $\overline{PD}_{fin}^{atual} = 0,197$ derivado da base elegível. Esse comportamento tem explicação direta na função objetivo: o coeficiente $c_k = \pi_k(\bar{u}\,t\,T - PD_k\cdot\mathrm{LGD})$ torna-se negativo quando $PD_k$ ultrapassa o limiar de 0,361, fazendo com que o próprio otimizador atribua $L_k = 0$ a 123 dos 800 clusters — sem que R1 precisasse atuar como restrição vinculante. O teto operacional (R3) não foi atingido por nenhum cluster, e a verificação de concentração pós-otimização (R4) registrou participação máxima de 3,6% para um único cluster, dentro do limite de 5%. O mesmo padrão qualitativo se repetiu em M3, com R2 ativa em 693 de 800 clusters e concentração máxima de 2,6%, indicando consistência estrutural da solução entre safras.
 
**Cobertura da base elegível.** O modelo recomendou oferta para 876.520 clientes em M1, correspondendo a 47,7% da base elegível, com limite médio de R\$ 607 por cliente. Em M3, a cobertura subiu para 55,9% dos elegíveis. Esses números contrastam fortemente com os 6,4% de cobertura da política vigente do parceiro sobre a base elegível de M1, revelando que a principal contribuição do modelo não está no ganho marginal de retorno sobre os clientes já atendidos, mas na extensão da oferta a uma parcela muito maior da base elegível respeitando o mesmo teto de risco da carteira.
 
**Sanidade econômica da solução.** A análise por cluster confirmou dois comportamentos esperados pela teoria: o limite recomendado cai à medida que o risco do cluster aumenta e se anula a partir do limiar de rentabilidade; e, para os clusters rentáveis, o limite recomendado coincide com o teto de capacidade $m_k \cdot CP_k$ (correlação de 0,996), confirmando que R2 é o fator que efetivamente governa a alocação.
