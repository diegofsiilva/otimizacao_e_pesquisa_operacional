...
A definição de limites de crédito pré-aprovados para grandes carteiras exige um processo consistente que equilibre retorno e risco, sobretudo em um cenário de inadimplência persistente no mercado brasileiro. Este trabalho propõe um pipeline reprodutível que segmenta clientes elegíveis em perfis homogêneos por meio de uma árvore de decisão CART e, em seguida, formula um problema de Programação Linear para definir o limite de cada segmento, maximizando o retorno líquido esperado (receita de interchange menos perda esperada por inadimplência) sob restrições de risco agregado, capacidade de pagamento e tetos operacionais. O problema é resolvido por uma implementação própria do método Simplex, validada por comparação cruzada com o solver CBC via PuLP. Aplicado às safras reais do parceiro (M1 e M3, com até 3,1 milhões de clientes elegíveis), o modelo elevou em 3,4% o retorno líquido esperado sobre a mesma população atendida pela política vigente, mantendo o risco da carteira praticamente estável, e ampliou o alcance de oferta de 6,4% para 47,7% dos elegíveis, respeitando todas as restrições prudenciais. Conclui-se que a abordagem é viável e auditável, controla o risco de forma endógena à função objetivo e abre caminho para extensões em programação inteira e segmentação adaptativa.
...
**Implementação própria do Simplex (motivação).** A decisão de implementar o método Simplex do zero, em vez de recorrer diretamente a uma biblioteca de otimização, atende a um objetivo pedagógico do módulo do curso, voltado à compreensão e à construção de algoritmos de otimização. Implementar o resolvedor torna explícitas as regras de pivoteamento, de entrada e saída da base e de parada, o que aumenta a transparência e a auditabilidade da decisão de crédito - atributos valorizados pelo parceiro. A biblioteca PuLP/CBC é mantida apenas como referência de validação cruzada, e não como o resolvedor principal.
