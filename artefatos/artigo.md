...
Assigning pre-approved credit limits at portfolio scale requires a methodology that simultaneously maximizes expected return, controls default risk, and satisfies prudential and operational constraints — a demand not adequately met by rule-based empirical approaches. This paper proposes and validates a reproducible pipeline composed of two articulated stages: (i) supervised segmentation of eligible customers via a CART decision tree, grouping homogeneous profiles along the dimension of expected marginal return; and (ii) formulation and resolution of a Linear Programming (LP) problem that determines, for each segment, the optimal pre-approved credit limit, maximizing net expected return — defined as expected interchange revenue minus expected credit loss — subject to exposure-weighted default risk constraints (R1), payment capacity leverage limits per segment (R2), and an operational ceiling per offer (R3). Default probability is calibrated by risk deciles to correct for selection biases present in historical data. The LP is solved by a custom implementation of the Simplex method with Bland's rule, validated by cross-comparison against the CBC solver via PuLP. Applied to two real and independent data cohorts, comprising 1.8 and 3.1 million eligible customers respectively, the pipeline increased expected net return by 3.4% over the subpopulation served by the incumbent policy, while keeping exposure-weighted default probability virtually unchanged (0.2 percentage point variation), and expanded credit offer coverage from 6.4% to 47.7% of eligible customers in the first cohort and 55.9% in the second, respecting all established prudential constraints. Analysis of active constraints revealed that the binding factor is individual payment capacity (R2), not the aggregate risk ceiling (R1) — a policy insight detectable only through an explicit and interpretable formulation. Numerical validation confirmed identical limits across all 800 production-instance segments when compared to the reference solver, with residual difference on the order of 10⁻⁸. Results demonstrate that the proposed approach is viable, auditable, and scalable, with risk control endogenous to the objective function, opening pathways for extensions in mixed-integer programming and adaptive cross-cohort segmentation.

**Keywords:** linear programming; pre-approved credit limit; supervised segmentation; CART decision tree; Simplex method; credit portfolio optimization; probability of default; risk calibration; credit card; payment capacity.

---

## RESUMO

A concessão de limites de crédito pré-aprovados em escala de portfólio exige metodologia que equilibre, de forma simultânea, maximização de retorno, controle de inadimplência e observância de restrições prudenciais e operacionais — demanda não satisfatoriamente atendida por abordagens baseadas em regras empíricas fixas. Este trabalho propõe e valida um pipeline reprodutível composto por duas etapas articuladas: (i) segmentação supervisionada de clientes elegíveis por meio de uma árvore de decisão CART, que agrupa perfis homogêneos na dimensão do retorno marginal esperado; e (ii) formulação e resolução de um Problema de Programação Linear (PL) que determina, para cada segmento, o limite ótimo de crédito pré-aprovado, maximizando o retorno líquido esperado — definido como a receita esperada de _interchange_ menos a perda esperada por inadimplência — sujeito a restrições de risco agregado ponderado por exposição (R1), alavancagem em relação à capacidade de pagamento por segmento (R2) e teto operacional por oferta (R3). A probabilidade de inadimplência é calibrada por decis de risco para corrigir vieses de seleção presentes nos dados históricos. O PL é resolvido por uma implementação própria do método Simplex com regra de Bland, validada por comparação cruzada com o solver CBC via PuLP. Aplicado a duas safras reais e independentes, com 1,8 e 3,1 milhões de clientes elegíveis, respectivamente, o pipeline elevou o retorno líquido esperado em 3,4% sobre a subpopulação atendida pela política vigente, mantendo a inadimplência financeira ponderada por exposição praticamente estável (variação de 0,2 ponto percentual), e ampliou a cobertura de oferta de 6,4% para 47,7% dos clientes elegíveis na primeira safra e 55,9% na segunda, respeitando todas as restrições prudenciais estabelecidas. A análise das restrições ativas revelou que o fator governante da solução é a capacidade de pagamento individual (R2), e não o teto de risco agregado (R1) — implicação de política identificável apenas por meio de formulação explícita e interpretável. A validação numérica confirmou limites idênticos em todos os 800 segmentos da instância de produção quando comparado ao solver de referência, com diferença residual da ordem de 10⁻⁸. Os resultados demonstram que a abordagem proposta é viável, auditável e escalável, com controle de risco endógeno à função objetivo, abrindo caminho para extensões em programação inteira mista e segmentação adaptativa entre safras.

**Palavras-chave:** programação linear; limite de crédito pré-aprovado; segmentação supervisionada; árvore de decisão CART; método Simplex; otimização de carteira de crédito; probabilidade de inadimplência; calibração de risco; cartão de crédito; capacidade de pagamento.
...
A escolha dessa abordagem é motivada diretamente pelo contexto operacional do parceiro, que precisa definir limites pré-aprovados em larga escala com (i) restrições prudenciais explícitas (teto de risco agregado e alavancagem por capacidade de pagamento), (ii) limites operacionais por oferta e (iii) rastreabilidade/auditabilidade das decisões. Além disso, o parceiro já possui uma política vigente de oferta, o que permite avaliar ganhos de retorno e controle de risco comparando as recomendações do modelo com o baseline observado (campo limite_ofertado). Os resultados desta etapa confirmam a viabilidade da abordagem: sobre a mesma população já atendida pela política vigente, o modelo elevou em 3,4% o retorno líquido esperado mantendo o risco da carteira praticamente estável, e ampliou a cobertura de oferta de 6,4% para 47,7% da base elegível sem ultrapassar o teto de risco vigente.
...
| $k$                           | Índice do segmento                                             | $k \in \{1,\dots,K\}$  | —                                                                                                                                            | —                             |
...
**Implementação própria do Simplex (motivação).** A decisão de implementar o método Simplex do zero, em vez de recorrer diretamente a uma biblioteca de otimização, é motivada por dois objetivos complementares. O primeiro é de transparência algorítmica: ao implementar o resolvedor, tornam-se explícitas as regras de pivoteamento, de entrada e saída da base e de critérios de parada, o que permite auditar cada etapa da solução e rastrear a origem de qualquer decisão de limite — requisito relevante em contextos regulados como o de crédito ao consumidor. O segundo é de independência de dependências externas: um resolvedor próprio elimina riscos associados a mudanças de licença ou de comportamento em versões futuras de bibliotecas de terceiros. A biblioteca PuLP/CBC é mantida exclusivamente como referência de validação cruzada dos resultados, e não como o resolvedor principal do pipeline.
...
- **scikit-learn:** modelos e rotinas de pré-processamento utilizados na etapa de segmentação (por exemplo, a árvore de decisão CART e transformações como imputação, padronização e codificação _one-hot_ quando aplicável).
...
**Tabela 4 — Aderência às restrições (R0–R3) e à regra de pós-otimização (safra M1)**
...
O coeficiente unitário $c_k = \pi_k(\bar{u}\,t\,T — PD_k\cdot\mathrm{LGD})$ vale aproximadamente $0{,}149$ para A, $0{,}052$ para B e $-0{,}036$ para C — este último negativo porque $PD_C = 0{,}45$ excede o limiar de 0,361. A solução ótima pode, então, ser antecipada por inspeção: o segmento C destrói valor e recebe $L_C = 0$; A e B são rentáveis e têm o limite empurrado até o teto de capacidade (R2), resultando em $L_A = \text{R\$ }3.000$ e $L_B = \text{R\$ }1.000$. Essa solução respeita o teto de risco financeiro (R1), cujo lado esquerdo vale $1.000(0{,}05-0{,}25)\cdot 3.000 + 2.000(0{,}20-0{,}25)\cdot 1.000 = -700.000 \le 0$ — isto é, R1 permanece com folga. O retorno correspondente é $Z^* = 1.000\cdot 0{,}14925\cdot 3.000 + 2.000\cdot 0{,}05150\cdot 1.000 = \text{R\$ }550.750$, sobre uma exposição total $E = \text{R\$ }5.000.000$.
