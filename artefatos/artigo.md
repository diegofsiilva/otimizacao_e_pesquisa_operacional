...
| `score_generico_1`           | Score de bureau (bureau 1)                    | min=49, med=409, max=995. Nulls: 0,1%                          | Variável de segmentação para a segmentação              |
| `score_generico_2`           | Score de bureau (bureau 2)                    | min=1, med=713, max=942. Nulls: <0,01%                         | Variável de segmentação para a segmentação              |
...
| $\alpha$                      | Concentração máxima de exposição em um único segmento           | $[0,1]$                | Constante (ex.: 5%) para as restrições                                                                                                      | Política/prudencial           |
...
isto é, limites abaixo de R\$ 200 são interpretados como “sem oferta”, e os demais são arredondados ao múltiplo de R\$ 50 mais próximo. O resultado final é então reportado por segmento e pode ser propagado ao nível individual via mapeamento do cliente ao seu respectivo `cluster_id`.
