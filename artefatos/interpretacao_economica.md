...
O modelo **não prioriza volume bruto**. Se esse fosse o objetivo, ele tentaria ampliar a base atendida independentemente da qualidade do risco. O que ocorre é diferente: na safra M1 completa, a política vigente alcança 117.367 clientes com `limite_ofertado` registrado (6,4% dos elegíveis), enquanto o modelo recomenda oferta para 876.520 clientes (47,7% dos elegíveis), mas mantendo a PD ponderada da carteira otimizada em 0,067, abaixo do teto de 0,197 (`artigo.md §4.1-4.2`). Há expansão de alcance, mas ela é condicionada à qualidade econômica da exposição.

A função objetivo é **orientada por margem**, porque maximiza o retorno líquido esperado: interchange menos perda esperada. Isso aparece no backtesting comparável da safra M1: sobre os mesmos 117.367 clientes já atendidos pelo parceiro, o retorno líquido esperado sobe de R\$ 4,90 milhões para R\$ 5,06 milhões (+3,4%), enquanto a PD ponderada por exposição muda pouco, de 0,0467 para 0,0486 (+0,2 p.p.) (`artigo.md §4.2`). O modelo, portanto, não aumenta limite por aumentar; ele realoca limite onde a margem ajustada ao risco compensa.

Ao mesmo tempo, a solução final é **governada por segurança**. A evidência mais forte é R2: em M1, a capacidade de pagamento fica ativa em 677 de 800 segmentos, e nenhum segmento atinge o teto operacional de R\$ 25 mil (`artigo.md §4.1`). Na leitura por decis, D1-D7 recebem algum limite, mas o valor cai de aproximadamente R\$ 258 em D1 para R\$ 9 em D7 e zera em D8-D10 (`modelagem_matematica.md §4`). A carteira recomendada não é agressiva: ela é **equilibrada com viés conservador**, pois busca margem, amplia cobertura, mas só concede volume quando a capacidade de pagamento e o risco calibrado sustentam a decisão.

### 3.3 Há evidência de seleção adversa ou exclusão de perfis?

Há evidência clara de **exclusão econômica de perfis de alto risco**, mas ela é racional dentro da função objetivo. Na solução por decis, D8, D9 e D10 recebem $L_k^* = 0$, mesmo tendo propensão maior que os decis de menor risco ($\pi_k$ de 0,455, 0,479 e 0,498). O motivo é que a perda esperada cresce mais do que a receita esperada: esses decis têm $PD_k$ de 0,716, 0,774 e 0,840, com custos reduzidos negativos de −R\$ 12.310,50, −R\$ 24.264,67 e −R\$ 37.201,42, respectivamente (`modelagem_matematica.md §4`). Forçar a entrada de D10, por exemplo, reduziria o retorno da carteira em vez de ampliá-lo.

Essa combinação também sugere um padrão de **seleção adversa potencial**: os perfis mais arriscados são justamente os que apresentam maior propensão estimada à contratação. Em D1, $\pi_k = 0{,}158$; em D10, $\pi_k = 0{,}498$. Se a decisão olhasse apenas conversão, a carteira tenderia a atrair mais clientes de alto risco. O papel econômico do modelo é bloquear essa armadilha: ele multiplica a propensão pelo spread líquido ajustado a PD, LGD e $\gamma_d$, de modo que maior vontade de contratar só gera limite quando não destrói valor esperado.

O cuidado é que essa exclusão não é neutra do ponto de vista de inclusão financeira. D8-D10 reúnem maior PD e menor capacidade prudencial ($CP_{p5}$ de R\$ 125, R\$ 100 e R\$ 75), ficando fora da oferta ou abaixo do limite mínimo operacional. Na prática, o modelo protege a carteira, mas também tende a negar crédito a perfis mais frágeis. A recomendação não é forçar limite nesses decis, e sim tratá-los com política própria: limites educativos, produto garantido, régua de entrada gradual ou campanhas de redução de risco antes de elegê-los ao cartão pré-aprovado tradicional.

Há ainda um viés de seleção nos próprios dados usados para calibrar risco. A inadimplência observada (`over30mob3`) só existe para quem contratou e ativou o produto: em M1, são 4.966 observações válidas e 377 eventos, dentro de uma base total de 14,6 milhões de clientes (`modelagem_matematica.md §1.2`). Nos decis altos, a evidência é ainda mais escassa: D8 e D9 têm zero observações de default observado, D10 tem apenas uma, e $\gamma_d$ precisa ser extrapolado (`modelagem_matematica.md §1.5.1`). Portanto, o modelo reduz seleção adversa no uso operacional, mas pode herdar a seleção da política vigente: como o banco quase não aprovou clientes de alto risco historicamente, há pouca evidência empírica para distinguir "alto risco real" de "alto risco pouco observado".
