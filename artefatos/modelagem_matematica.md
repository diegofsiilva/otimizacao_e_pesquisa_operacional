...
**Análise de sensibilidade a $\gamma_d$.** Após a calibração temporal ($T = 22$, Seção 1.5.1), o parâmetro de maior sensibilidade do modelo passou a ser $\gamma_d$. Uma elevação uniforme de 25 % em $\gamma_d$ (cenário de drift do modelo de scoring) reduz a margem $c_k$ de todos os clusters proporcionalmente e pode provocar a saída dos clusters de fronteira (D5–D7) da solução básica, deslocando exposição para D1–D4. O monitoramento empírico de $\gamma_d$ por safra é, portanto, o gatilho principal de reotimização do LP.
