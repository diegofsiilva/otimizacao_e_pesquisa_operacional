              ...
              {clusters.length > 0 ? (
                <div className="overflow-auto max-h-56">
                  <table className="w-full text-xs border-collapse">
                    <thead>
                      <tr className="text-[#9C9C9F] text-left border-b border-[#E8EFF7]">
                        <th className="pb-2 pr-3 font-medium">Cluster</th>
                        <th className="pb-2 pr-3 font-medium text-right">Simplex</th>
                        <th className="pb-2 pr-3 font-medium text-right">PuLP</th>
                    ...
                    <tbody>
                      {clusters
                        .slice()
                        .sort(function (a, b) { return b.limite_otimizado - a.limite_otimizado; })
                        .slice(0, 15)
                        .map(function (c) {
                          var match =
                            c.limite_otimizado_pulp != null &&
                            Math.abs(c.limite_otimizado - c.limite_otimizado_pulp) < 50;
                          return (
                            <tr key={c.cluster_id} className="border-b border-[#E8EFF7] last:border-0">
                              <td className="py-1.5 pr-3 font-medium text-[#3B4049]">
                                CLU-{c.cluster_id}
                              </td>
                              <td className="py-1.5 pr-3 text-right text-[#0D1B2A] font-semibold">
                                {c.limite_otimizado > 0
                                  ? "R$ " + Number(c.limite_otimizado).toLocaleString("pt-BR")
                                  : "—"}
                              </td>
                              <td className="py-1.5 pr-3 text-right text-[#3B4049]">
                                {c.limite_otimizado_pulp != null
                                  ? c.limite_otimizado_pulp > 0
                                    ? "R$ " + Number(c.limite_otimizado_pulp).toLocaleString("pt-BR")
                                    : "—"
                                  : "-"}
                              </td>
                              <td className="py-1.5 text-center">
                                {c.limite_otimizado_pulp != null ? (
                                  match ? (
                                    <span className="text-[#1A7A40] font-bold">✓</span>
                                  ) : (
                                    <span className="text-[#DC2F37] font-bold">✗</span>
                                  )
                                ) : (
                                  <span className="text-[#9C9C9F]">-</span>
                                )}
                              </td>
                            </tr>
                          );
