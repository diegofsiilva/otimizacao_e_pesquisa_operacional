    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={handleOverlay}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg modal-enter" onClick={function(e){ e.stopPropagation(); }}>
        ...
        <div className="flex items-center gap-3 px-6 py-4 border-b border-slate-100">
          ...
          <button onClick={onClose} className="ml-auto p-1.5 rounded-lg text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition-colors">
          ...
          <div>
            ...
            <div className="space-y-3">
              {PARAMS_EDITAVEIS.map(function(p) {
                ...
                return (
                  <div key={p.key} className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                    ...
                    {isEditing && (
                      <div className="mt-4 space-y-3">
                        <div className="flex justify-between text-xs text-slate-500">
                          <span>{p.label}</span>
                          <span className="font-semibold text-sky-600">{formatVal(p.key, params[p.key])}</span>
                        </div>
                        ...
                        <div className="flex gap-2">
                          <button onClick={function(){ setEditing(null); }}
                            className="flex-1 py-1.5 rounded-lg text-xs font-semibold bg-green-500 text-white hover:bg-green-600 transition-colors">
                            Salvar
                          </button>
                          <button onClick={function(){ handleCancel(p.key); }}
                            className="flex-1 py-1.5 rounded-lg text-xs font-semibold bg-slate-300 text-slate-700 hover:bg-slate-400 transition-colors">
                            Cancelar
                          </button>
                        </div>
                      </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {PARAMS_NAO_EDITAVEIS.map(function(p, i) {
                return (
                  <div key={i} className="rounded-xl bg-slate-100 px-4 py-3">
